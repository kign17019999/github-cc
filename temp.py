...
def main():
    ...
    while True:
        ...
        if time.time()-no_msg_time > time_before_resend:
            ...
            for key, value in dict_of_matrixs.items():
                ...
                if time.time()-start_time > time_before_print_process:
                    start_time = time.time()
                    print(f'trying to sending...{count_resend}/{partition} ')
                print(f'trying to sending...{count_resend}/{partition} ')
            if count_resend > 0:
                no_msg_time = time.time()
            else:
    ...
    print('-----------------------------------------------')
    print(f'>> total time sending message      : {stop_time_for_sending-start_time_for_sending}')
    print(f'>> total time getting result       : {stop_time_getting_result-start_time_getting_result}')
    print('-----------------------------------------------')
    print(f'>> total time distibuted system    : {stop_time_all-start_time_all}')
    print(f'>> total time local system         : {stop_time_local-start_time_local}')
