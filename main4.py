...
def main():
    ...
    randF = 0
    randT = 10
    num_for_each_worker = np.zeros(available_worker)
    ...
    for i in range(partition):
        ...
        num_for_each_worker[i%available_worker]+=1
        print(f'message_id: {message_id} to worker {i%available_worker}')
    print(f'fum: {num_for_each_worker}')
    
    print('start receving & processing...')
    for i in range(available_worker):
        print(f'trying in worker {i}')
        while num_for_each_worker[i]!=0:
